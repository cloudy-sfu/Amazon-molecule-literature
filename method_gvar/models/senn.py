...
class SENNGC(nn.Module):
    def __init__(self, num_vars: int, order: int, hidden_layer_size: list, num_hidden_layers: int, device: torch.device,
                 method="OLS"):
        """
        ...
        for k in range(order):
            layers = [nn.Linear(num_vars, hidden_layer_size[0]), nn.ReLU()]
            for i in range(num_hidden_layers - 1):
                layers += [nn.Linear(hidden_layer_size[i], hidden_layer_size[i+1]), nn.ReLU()]
            layers += [nn.Linear(hidden_layer_size[-1], num_vars ** 2)]
