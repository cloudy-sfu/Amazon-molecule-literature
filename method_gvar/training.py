...
def run_epoch(model: nn.Module, optimiser: optim, predictors: np.ndarray, responses: np.ndarray,
              time_idx: np.ndarray, criterion: torch.nn.modules.loss, lmbd: float, gamma: float, batch_size: int,
              device: torch.device, alpha=0.5, train=True):
    """
...
def training_procedure(data, order: int, hidden_layer_size: list, end_epoch: int, batch_size: int, lmbd: float,
                       gamma: float, seed=42, num_hidden_layers=1, initial_learning_rate=0.001, beta_1=0.9,
                       beta_2=0.999, use_cuda=True, test_data=None, name=''):
    """
    ...
    if use_cuda:
        if torch.cuda.is_available():
            ...
            raise Exception("`use_cuda=True`, but CUDA is not available.")
    else:
    ...
    for epoch in tqdm(range(end_epoch), desc=name):
        # Train
        run_epoch(model=senn, optimiser=optimiser, predictors=predictors, responses=responses,
    ...
    with torch.no_grad():
        coeffs, l, mse, pen1, pen2 = run_epoch(model=senn, optimiser=optimiser,
        ...
        if test_data is not None:
            ...
            coeffs_test, l_test, mse_test, pen1_test, pen2_test = run_epoch(model=senn,
...
def training_procedure_stable(data, order: int, hidden_layer_size: list, end_epoch: int, batch_size: int, lmbd: float,
                              gamma: float, seed=42, initial_learning_rate=0.001, beta_1=0.9,
                              beta_2=0.999, Q=20, use_cuda=True):
    """
    ...
    a_hat_1, coeffs_full_1 = training_procedure(data=[data_1], order=order, hidden_layer_size=hidden_layer_size,
                                                ...
                                                beta_2=beta_2, use_cuda=use_cuda, name=f'data=1/3,λ={lmbd},γ={gamma}')
    a_hat_2, coeffs_full_2 = training_procedure(data=[data_2], order=order, hidden_layer_size=hidden_layer_size,
                                                end_epoch=end_epoch, lmbd=lmbd, gamma=gamma, batch_size=batch_size,
                                                seed=seed, num_hidden_layers=num_hidden_layers,
                                                initial_learning_rate=initial_learning_rate, beta_1=beta_1,
                                                beta_2=beta_2, use_cuda=use_cuda, name=f'data=2/3,λ={lmbd},γ={gamma}')
    ...
    a_hat, coeffs_full = training_procedure(data=data, order=order, hidden_layer_size=hidden_layer_size,
                                            ...
                                            seed=seed, num_hidden_layers=num_hidden_layers,
                                            initial_learning_rate=initial_learning_rate, beta_1=beta_1,
                                            beta_2=beta_2, use_cuda=use_cuda, name=f'data=3/3,λ={lmbd},γ={gamma}')
    q = np.quantile(a=a_hat, q=alpha_opt)
    a_hat_binary = (a_hat >= q) * 1.0
    return a_hat_binary, coeffs_full


def training_procedure_trgc(data, order: int, hidden_layer_size: list, end_epoch: int, batch_size: int, lmbd: float,
                            gamma: float, seed=42, initial_learning_rate=0.001, beta_1=0.9,
                            beta_2=0.999, Q=20, use_cuda=True, true_struct=None,
                            signed=False):
    """
    ...
    a_hat_1, coeffs_full_1 = training_procedure(data=[data_1], order=order, hidden_layer_size=hidden_layer_size,
                                                ...
                                                seed=seed, num_hidden_layers=num_hidden_layers,
                                                initial_learning_rate=initial_learning_rate, beta_1=beta_1,
                                                beta_2=beta_2, use_cuda=use_cuda, name=f'data=1/2,λ={lmbd},γ={gamma}')
    a_hat_2, coeffs_full_2 = training_procedure(data=[data_2], order=order, hidden_layer_size=hidden_layer_size,
                                                end_epoch=end_epoch, lmbd=lmbd, gamma=gamma, batch_size=batch_size,
                                                seed=seed, num_hidden_layers=num_hidden_layers,
                                                initial_learning_rate=initial_learning_rate, beta_1=beta_1,
                                                beta_2=beta_2, use_cuda=use_cuda, name=f'data=2/2,λ={lmbd},γ={gamma}')
